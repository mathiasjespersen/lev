...
export async function generateMetadata(props: Props, parent: ResolvingMetadata): Promise<Metadata> {
  ...
  const ogImage = resolveOpenGraphImage(post?.thumbnail?.image)
          ...
          <div>
            <div className="pb-6 grid gap-6 mb-6 border-b border-gray-100">
              <div className="max-w-3xl flex flex-col gap-6 items-start">
                ...
                {post.category && <span className="text-sm text-gray-600 bg-gray-200 px-2 py-1 rounded">{post.category}</span>}
              </div>
            </div>
            <article className="gap-6 grid max-w-4xl">
              {post.thumbnail && <Visual visual={post.thumbnail} />}
              {post.content?.length && (
                <PortableText
